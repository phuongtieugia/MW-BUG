...
window.FreeGift.$(document).ready(function($){
    ...
    FreeGift.Views.InitInSlider     = Backbone.View.extend({
        ...
        beforeAddToCart: function(params){
            ...
            if(window.FreeGift.$(tableSelectorCart).find("tbody").find("tr").length > 0){
                ...
                colName.append('<span class="product-adding">'+Translator.translate('Добавляем...')+'</span>');
                ...
                window.staticLoaderGift.overlayShow({text: "Добавляем подарок в корзину..."});
    ...
    FreeGift.Views.CheckoutCart     = Backbone.View.extend({
        ...
        beforeAddToCart: function(params){
            ...
            if(window.FreeGift.$(tableSelectorCart).find("tbody").find("tr").length > 0){
                ...
                colImage.html('<a href="javascript:;" title="'+params.p_name+'" class="product-image"><img src="'+params.image+'" width="75" height="75"/></a>');
                colName.html('<h2 class="product-name adding"><a href="javascript:;">'+params.p_name+'</a><br /></h2>');
                colName.append('<span class="product-adding">'+Translator.translate('Обновляем...')+'</span>');
            }
        },
        afterAddToCart: function(params){
            var data = params.data;
        ...
        hdlRemove: function(ev){
            ...
            window.staticLoaderGift.overlayShow({text: "Удаляем подарок..."});
            ...
            window.FreeGift.$.ajax({
                ...
                success: function(data){
                    if(data.error == "1"){
						...
						/* ptg code */
						var mwhref = window.FreeGift.$(ev.target).attr("href");
						$('#cart-sidebar a.btn-remove').filter('[href="'+mwhref+'"]').parents('li.item').slideUp('slow');
						//alert(mwhref);
						$(".sidebar .block-content .summary").slideUp("slow");
						//$(".block-cart-top .summary").slideUp("slow");
						$.ajax({
							type : 'POST',
							url  : mwfrg,
							data : 'ajax=true',
							dataType: 'json',
							success: function(data){
								$(".sidebar .block-content .summary").html(data.sidebar).slideDown("slow");
								$(".block-cart-top .summary").html(data.top);
							}
						});
						/* ptg endcode */
    ...
    FreeGift.Views.miniUpdCart      = Backbone.View.extend({
        ...
        process: function(params){
            ...
            window.staticLoaderGift.overlayShow({text: "Обновляем корзину..."});
            view_freegift_init.undelegateEvents();
            window.FreeGift.$.ajax({
                type : 'POST',
