...
function postcodeondelivery(_postcode){
	var ddateurl = mw_baseUrl+"onestepcheckout/index/getPostday";
	var ddatespecialurl = mw_baseUrl+"onestepcheckout/index/getPostSpecial";
	var params = [];
		params.push({name:'dpostcode',value:_postcode});
	window.OneStep.$.ajax({
		type    :   "POST",
		url     :   ddateurl,
		data    :   params,
		...
		success: function(data){
			...
			if(data!=null){
				var str = data.split(",");
				jQuery('.cpostdate').addClass("disablerow");
				jQuery(".ddate_day_active").removeClass();
				
				if(jQuery('li[data-day*="'+str+'"]').length > 0){
					jQuery('li[data-day*="'+str+'"]').removeClass("disablerow");
				}
				for(i = 0;i < str.length;i++){
						jQuery('li[data-day*="'+str[i]+'"]').removeClass("disablerow");
				...
				window.OneStep.$.ajax({
					...
					success: function(_data){
						...
						if(_data!=null){
							//jQuery('.cpostdate').addClass("disablerow");
							if(jQuery('li[data-slot*="'+_data+'"]').length > 0){
								//jQuery('li[data-slot*="'+_data+'"]').removeClass("disablerow");
								jQuery("li[data-slot!='"+_data+"']").addClass("disablerow");
							}
				...
				jQuery('.cpostdate').removeClass("disablerow");
			}
