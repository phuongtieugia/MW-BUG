        ...
        if(Mage::getModel('ddate/dtime')->getCollection()->count() > 0 && Mage::helper('ddate')->haveAnySlotAvailable()) {
            ...
            if ($this->getRequest()->isPost()) {
                ...
                */
                ...
                $_shipping = Mage::getSingleton('checkout/session')->getQuote()->getShippingAddress()->getShippingMethod();
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (count($slot)) {
                foreach($slot as $key => $sl) {
                    ...
                    $ordered = Mage::helper('ddate')->ordered_counting($day[0],$sl->getDtimeId());
                    if($ordered && $ordered >= $sl->getMaximumBooking()){
                        unset($slot[$key]);
                    }
                };
            }
