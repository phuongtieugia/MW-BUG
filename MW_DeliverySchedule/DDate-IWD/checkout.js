...
IWD.OPC = {
		...
		preparePaymentResponse: function(response){
			...
			// Ddate
				var form = $j('#co-ddate-form').serializeArray();
				IWD.OPC.Checkout.xhr = $j.post(IWD.OPC.Checkout.config.baseUrl + 'onepage/json/saveDdate',form, IWD.OPC.prepareDdateResponse,'json');
				//IWD.OPC.saveOrder();				
			}
			else
			{
		...
		prepareDdateResponse: function(response){
...
IWD.OPC.Checkout = {
		...
		prepareAddressResponse: function(response){
			...
			if (typeof(response.payments) != "undefined"){
				...
				IW.OPC.removeNotAllowedPaymentMethods();
...
IWD.OPC.Billing = {
		...
		validateAddressForm: function(form){
			...
			$j('#opc-address-form-billing .required-entry').each(function(){
				if($j(this).val() == '' && $j(this).css('display') != 'none' )
					is_empty = true;
			});
			if(is_empty)
				return false;
