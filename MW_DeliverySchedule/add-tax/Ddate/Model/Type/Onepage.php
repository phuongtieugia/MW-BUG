...
class MW_Ddate_Model_Type_Onepage extends Mage_Checkout_Model_Type_Onepage
{
    ...
    public function saveDdate($data)
    {
		...
		// get Tax
		$tax = Mage::getModel('ddate/dtime')->load($data['dtime'])->getDtimeTax();
		if($tax > 0){
			Mage::getSingleton('core/session')->setDeliveryMwfee($tax);
		}
		else{
			Mage::getSingleton('core/session')->setDeliveryMwfee(0);
		}
		$this->getQuote()->collectTotals()->save();
        $this->getQuote()->setDdate($data['date']);
        $this->getQuote()->setDdateTax($tax);
    ...
    public function saveOrder()
    {
        ...
        if ($order) {
			...
			$tax = Mage::getModel('ddate/dtime')->load($dtime)->getDtimeTax();
            if ($ddates->count() > 0) {
