	...
	public function _getCustomer($customerId,$order){
        ...
        if ($customerId) {
			...
			if ($customer) {    				 
				...
				$customerInfo['city'] = (isset($address['city']) ? $address['city'] : '');

				$customerInfo['state'] = (isset($address['region']) ? $address['region'] : '');

				$customerInfo['zip_code'] = (isset($address['postcode']) ? $address['postcode'] : '');

				$countryName = Mage::getModel('directory/country')->load((isset($address['country_id']) ? $address['country_id'] : ''))->getName(); 
            ...
            foreach(unserialize($rule['email_chain']) as $emailChainItem)
            {
                ...
                if(count($birthDays))
                {
                    ...
                    {						
                        ...
                        if($this->_checkExistQueueEmail($rule['rule_id'],0,$birthDay['email'],$emailTemplate['code'],1)){
