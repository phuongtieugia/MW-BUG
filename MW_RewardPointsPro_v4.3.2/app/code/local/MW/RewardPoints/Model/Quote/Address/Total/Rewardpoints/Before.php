        ...
        public function collect(Mage_Sales_Model_Quote_Address $address)
        {
            ...
            if((int)Mage::helper('rewardpoints')->getRedeemPointsOnTax($store_id) == MW_RewardPoints_Model_Redeemtax::BEFORE)
            {
                ...
                foreach ($items as $item)
                {
                    ...
                    switch ($product->getTypeId())
                    {
                        ...
                        case 'configurable':
                            ...
                            $mw_reward_point_earn = $product->getData('reward_point_product');
                            if($mw_reward_point_earn > 0)
                            {
                                $earn_reward_point = $earn_reward_point + $qty * $mw_reward_point_earn;
                            }
                            else
                            {
                                if($info = $item->getProduct()->getCustomOption('info_buyRequest'))
                                ...
                                foreach ($infoArr['super_attribute'] as $attr_id => $val)
                                {
                                    ...
                                    $earn_reward_point += intval($collection->getEarnPoint());
                                }
                            }
                            //$earn_reward_point = $earn_reward_point + $product_earn_point;
                ...
                if(!Mage::helper('rewardpoints')->getCouponRwpConfig($store_id))
                {
                    ...
                    {
                        $quote->setCouponCode('');
                        $quote->setSpendRewardpointCart($spend_reward_point_cart);
                    }
                    else
                    {
                        $quote->setSpendRewardpointCart($spend_reward_point_cart);
                ...
                if($quote->getMwRewardpoint() > $spend_reward_point_cart) {
                    $quote->setMwRewardpoint($spend_reward_point_cart);
                    Mage::getSingleton('core/session')->setMwRewardpointAfterDrop($spend_reward_point_cart);
                }
                
                $quote->setEarnRewardpoint($earn_reward_point);
                $quote->setEarnRewardpointCart($earn_reward_point_cart);
                $quote->setMwRewardpointSellProduct($product_sell_point);
                $quote->setMwRewardpointDetail(serialize($reward_point_detail));
                $quote->setMwRewardpointRuleMessage(serialize($array_rule_show));
