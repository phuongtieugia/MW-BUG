    ...
    class MW_RewardPoints_Model_Credit extends Varien_Object
    {
        ...
        public function exchange($argv)
        {
            ...
            try
            {
                if($rewardPoints < 0)
                {
                    throw('reward points is incorrect');
                }
                $_customer = Mage::getModel('rewardpoints/customer')->load($customer->getId());

                $_customer->addRewardPoint($rewardPoints);
                $historyData = array('type_of_transaction' => MW_RewardPoints_Model_Type::EXCHANGE_FROM_CREDIT, 'amount' => $rewardPoints, 'balance' => $_customer->getMwRewardPoint(), 'transaction_detail' => '', 'transaction_time' => Mage::getSingleton('core/date')->gmtDate(), 'status' => MW_RewardPoints_Model_Status::COMPLETE);
