...
protected function _prepareColumns()
	...
	/* protected function _myDdateFilter($collection, $column)
    {
		...
		//Mage::log($value,null,"ptg.log");
		$this->getCollection()->getSelect()->where("one_step.mw_deliverydate_date LIKE '%$value%'");
	...
	protected function _myDdateFilter($collection, $column)
    {
        ...
        if($from)
            $this->getCollection()->addFieldToFilter('one_step.mw_deliverydate_date', array('gteq' => $from));
        if($to)
            $this->getCollection()->addFieldToFilter('one_step.mw_deliverydate_date', array('lteq' => $to));
    }
}
