...
class Ebizmarts_SagePaySuite_PaymentController extends Mage_Core_Controller_Front_Action {
	...
	public function _MW_OSCSaveBilling()
    {
        ...
        $Q->setData('customer_email', $addressData['billing']['email']);
        $Q->setData('customer_firstname', $addressData['billing']['firstname']);
        $Q->setData('customer_lastname', $addressData['billing']['lastname']);
        ...
        // Save payment method
        if ($Q->isVirtual()) {
            $Q->getBillingAddress()->setPaymentMethod($paymentMethod);
        } else {
            $Q->getShippingAddress()->setPaymentMethod($paymentMethod);
        }
        ...
        if($shippingMethod == 'flatrate_flatrate') {
            ...
            foreach($methods as $_code => $_method) {
                if (strpos($shippingMethod, $_code) !== false) {
                    $title = Mage::getStoreConfig("carriers/$_code/title");
                    $method = Mage::getStoreConfig("carriers/$_code/name");
                    break;
                }
            }

            $shippingAddress->setShippingMethod($shippingMethod);
            $shippingAddress->setShippingDescription($title . ' - ' . $method);
        ...
        if (!Mage::helper('customer')->isLoggedIn()) {
            if (!empty($addressData['billing']['customer_password']) && !empty($addressData['billing']['confirm_password']) && ($addressData['billing']['customer_password'] == $addressData['billing']['confirm_password'])) {
                ...
                $Q->setCheckoutMethod('register');
                $Q->getCustomer()->setData('password', $password);
                $Q->setData('password_hash', Mage::getModel('customer/customer')->encryptPassword($password));
            }
        }
    }
