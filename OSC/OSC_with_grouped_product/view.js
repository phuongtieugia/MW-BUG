...
//          this.initFbApp();
        ...
        hdlForgotpass: function(ev){
            if(this.dataForm.validator && this.dataForm.validator.validate()){
                this.submitForgot(ev, this.$el.find('#email_address').val());
            }
        ...
        submitForgot: function(ev, email){
            var view = this;
            var self = $(ev.target);
            ...
            window.OneStep.$.ajax({
                ...
                success: function(msg){
                    if(msg == 1){
                        var fancybox = self.closest("#fancybox-wrap");
                        console.log(fancybox);
                        fancybox.find('#inline3').html("<div class='mw-osc-title-login'><h1>"+Translator.translate("Password is sent successfully!")+"</h1></div><div class='fieldset' style='padding: 10px 10px;'>"+Translator.translate("We have now sent you a new password to your email address. Click the link below to return to login.")+"</div><p class='back-link'><a id='backlogin1' href='#inline1' style='color:#1E7EC8;'><small>&laquo; </small>"+Translator.translate("Back to Login")+"</a></p>");
