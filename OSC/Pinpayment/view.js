    ...
    OneStep.Views.Init              = Backbone.View.extend({
        ...
        updateOrder: function(){
                ...
                )
            {
                ...
                return false;
            } else if(window.OneStep.$('input[name=payment\\[method\\]]:checked').val() == "pinpayments"){
                view_onestep_plugin_pin_payments.getToken(view);
                return false;
            }
                ...
                success :   function(data){
                    ...
                    if(typeof data.items != 'undefined'){
                        ...
                        if(data.items == ""){
                            ...
                            location.href = onestepConfig.url.cartlink;
                            return false;
                        }
                    }
...
//          this.initFbApp();
    ...
    OneStep.Plugins.PinPayments             = Backbone.View.extend({
        getToken: function(parentView) {
            var PINGATEWAY = new MWPinPayments(window['PINKEY']);
            PINGATEWAY.getToken(parentView);
        },
        save: function () {
            var view = window.OneStep.$("#onestep_form");
            view.find('.btn-checkout').attr('disabled','disabled').css("opacity", 0.3);
            view.find("#review-please-wait").show();
            window.OneStep.$.ajax({
                type    : "POST",
                url     : onestepConfig.url.updOrderMethod,
                data    : window.OneStep.$("#onestep_form").serialize(),
                dataType: "json",
                success: function(data){
                    view.find("#review-please-wait").hide();
                    view.find('.btn-checkout').removeAttr('disabled').css("opacity", 1);
                    if(data.error == 0){
                        if(data.msg.search("hosted_pro") != -1 || data.msg.search("payflow_link") != -1 || data.msg.search("payflow_advanced") != -1){
                            view.saveOrder();
                        }else{
    ...
    OneStep.Plugins.Storecredit             = Backbone.View.extend({
        el: window.OneStep.$("#mw-checkout-payment-storecredit"),
        initialize: function(){
        },
        updateStorecredit: function(){
            var credit = this.$el.find("#mw_storecredit_value").val();
            var params = {updatestorecredit: true, updatepaymenttype: false};
            params.addition_post = {storecredit: credit};
            view_onestep_init.update(params);
        }
    });
    ...
    view_onestep_plugin_pin_payments        = new OneStep.Plugins.PinPayments();
    gb_view_onestep_plugin_rewardpoints     = new OneStep.Plugins.RewardPoint();
    gb_view_onestep_plugin_storecredit      = new OneStep.Plugins.Storecredit();
    // gb_view_onestep_plugin_storecredit.updateStorecredit();
