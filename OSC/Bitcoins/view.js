...
window.OneStep.$(document).ready(function($){
    OneStep.Views.Payment           = Backbone.View.extend({
        ...
        hdlChangeMethod: function(ev){
            ...
            if(val != this.payment_method_changed){
                ...
                var params = {updatepaymenttype: (window.OneStep.$.inArray(val, regetItemPayment) == -1 ? false : true), updatepaymentmethod: (window.OneStep.$.inArray(val, regetItemPayment) == -1 ? false : true),updateshippingmethod: true,updateshippingtype:true};
           ...
           /*  view_onestep_load_session_form.bindAll(); */
            window.updateOnestep = window.OneStep.$.ajax({
                ...
                success :   function(data){
					...
					if(typeof data.mwitems_after != 'undefined'){
						window.OneStep.$("#mwitems_after").html(data.mwitems_after);
					}
                    window.OneStep.$('.btn-checkout').removeAttr('disabled');
    ...
    OneStep.Views.DeliveryDate      = Backbone.View.extend({
        ...
        hdlType: function(ev){
            var val = window.OneStep.$(ev.target).filter(':checked').val();
            console.log(val);
            console.log(this.delivery_changed);
