    ...
    OneStep.Views.Shipping          = Backbone.View.extend({
        ...
        hdlBlurRegion: function(ev){
            ...
            if(window.OneStep.$(ev.target).get(0).tagName == 'INPUT'){
                ...
                if (this.val_regionship_before != val) {
        ...
        hdlBlurRegion: function(ev){
            if(onestepConfig.ajaxRegion){
                ...
                if(window.OneStep.$(ev.target).get(0).tagName == 'INPUT'){
                    ...
                    if (this.val_regionbill_before != val) {
...
//alert(onestepConfig.validVAT) ;
    ...
    OneStep.Views.Coupon            = Backbone.View.extend({
        ...
        hdlSubmitCoupon: function(ev){
            ...
            if(val == ""){
                alert(Mwosctranslator.translate("Please enter coupon code."));
                return false;
            }
            this.update(val, 0);
        },
    ...
    OneStep.Views.Referal           = Backbone.View.extend({
        ...
        hdlSubmitReferal: function(ev){
            ...
            if(val == ""){
                alert(Mwosctranslator.translate("Please enter referral code."));
                return false;
            }
            this.update(val, 0);
            return true;
    ...
    OneStep.Views.Init              = Backbone.View.extend({
        ...
        initialize: function(){
            ...
            /*window.OneStep.$("#mw_paypalpro").fancybox({
                ...
                onClosed            :function(){
                    window.location = mw_baseUrl;
                    jQuery("#checkout-paypaliframe-load").html("");
                    jQuery("#checkout-paypaliframe-load").hide();
                }
            });*/
        ...
        checkVAT: function(cc, vn){
            ...
            if(flsg == 1){
                verifiedVAT = true;
                /* var params = {checkvat:true, updatebillingaddress: true}; */
                var params = {changebycountry:true,checkvat:true,updatebillingaddress:true,updateshippingtype:true,updatepaymenttype:true};
        ...
        afterValidatedVAT: function(data){
			/* console.log($(data).find('valid').text()); */
            /* var xml = data,
            ...
            if(result == "true")
            {
                ...
                window.OneStep.$("#osc_billing_vat_id label").append(" <b id='mw_osc_vlvat_text'>(<span style='color:green; font-weight:bold'> "+Mwosctranslator.translate("Verified")+" </span>) </b>");
                window.OneStep.$("#osc_shipping_vat_id label").append(" <b id='mw_osc_vlvat_text'>(<span style='color:green; font-weight:bold'> "+Mwosctranslator.translate("Verified")+" </span>) </b>");
            ...
            else
            {
                ...
                window.OneStep.$("#osc_billing_vat_id label").append(" <b id='mw_osc_vlvat_text_error'> (<span style='color:red; font-weight:bold' > "+Mwosctranslator.translate("Not Verified")+" </span>)</b>");
                window.OneStep.$("#osc_shipping_vat_id label").append(" <b id='mw_osc_vlvat_text_error'> (<span style='color:red; font-weight:bold' > "+Mwosctranslator.translate("Not Verified")+" </span>)</b>");
                window.OneStep.$('.btn-checkout').removeAttr('disabled');
            }
        },
        ...
        hdlBtnCheckout: function(ev){
            ...
            if(document.getElementById('ddate:date') != null)
            {
                if(document.getElementById('ddate:date').value=='')
                {
                    ...
                    {
                        window.OneStep.$('#co-ddate').append('<dt id="ddate-osc"><div class="validation-advice" id="advice-required-entry_delivery" style="">'+Mwosctranslator.translate('Please select a delivery time!')+'</div></dt>');
                    }
            ...
            if(window.OneStep.$('input[name=payment\\[method\\]]').filter(':checked').length == 0){
                if(!window.OneStep.$('#advice-required-entry_payment').length){
                    window.OneStep.$('#checkout-payment-method-load').append('<dt><div class="validation-advice" id="advice-required-entry_payment" style="">'+Mwosctranslator.translate('Please select a payment method for your order!')+'</div></dt>');
                }
            }else{
            ...
            if(window.OneStep.$('input[name=shipping_method]').filter(':checked').length == 0){
                if(!window.OneStep.$('#advice-required-entry_shipping').length){
                    window.OneStep.$('#checkout-shipping-method-loadding').append('<dt><div class="validation-advice" id="advice-required-entry_shipping" style="">'+Mwosctranslator.translate('Please select a shipping method for your order!')+'</div></dt>');
                    window.OneStep.$('#group-select').append('<dt><div class="validation-advice" id="advice-required-entry-delivery" style="">'+Mwosctranslator.translate('Please select delivery time!')+'</div></dt>');
                }
                if(window.OneStep.$('input[name=shipping_method]').length == 0){
                    if(!window.OneStep.$('#advice-required-entry_shipping').length){
                        window.OneStep.$('#checkout-shipping-method-load').append('<dt><div class="validation-advice" id="advice-required-entry_shipping" style="">'+Mwosctranslator.translate('Your order cannot be completed at this time as there is no shipping methods available for it. Please make necessary changes in your shipping address.')+'</div></dt>');
                    }
                }
            }else{
            ...
            if(!form.validator.validate()){
                ...
                if(isvat == 0){
                    alert(Mwosctranslator.translate("TaxVat Number is not verified"));
                    return false;
                }
        ...
        updateEmailmsg: function(val){
            ...
            window.OneStep.$.ajax({
                ...
                success: function(msg){
                    ...
                    var error = "<ul class=\"messages\"><li class=\"error-msg\"><ul><li>"+Mwosctranslator.translate("There is already a customer registered using this email address. Please login using this email address or enter a different email address.")+"</li></ul></li></ul>";
                    window.OneStep.$('.btn-checkout').removeAttr('disabled');
                    if(msg == 0){
        ...
        saveOrder: function(){
            window.OneStep.$('#message-box').html('');
            var form_key = window.OneStep.$("#onestep_form").attr('data-key');
            window.OneStep.$.ajax({
                type    : "POST",
                /*url     : onestepConfig.url.saveOrder,*/  /** ?!?!? Not exist method in controller, who support */
                url     : onestepConfig.url.saveOrder+"form_key/"+form_key+"/",
                data    : window.OneStep.$("#onestep_form").serialize(),
                success: function(msg){
                    ...
                    if(str.update_section !== undefined ){
                        ...
                        window.OneStep.$("#checkout-paypaliframe-load").show("slow");
                        $('#mw_paypalpro').trigger('click');
                        //window.OneStep.$("#checkout-paypaliframe-load").html(str.update_section.html);
                    }
            ...
            window.updateOnestep = window.OneStep.$.ajax({
                ...
                success :   function(data){
                    ...
                    if(typeof data.coupon != 'undefined' && data.coupon != ""){
                        window.OneStep.$("#tab-coupon").html(data.coupon.html);
                    }
    ...
    OneStep.Views.Loader            = Backbone.View.extend({
        ...
        show: function(params){
            ...
            if(params.show_text){
                window.OneStep.$("#"+params.el).append("<div style='text-align: center; color: #757373;'>"+Mwosctranslator.translate("Please wait..")+".</div>");
            }
        },
    ...
    OneStep.Views.DeliveryDate      = Backbone.View.extend({
        ...
        editTimepicker: function(isTimeRange){
            ...
            if(isTimeRange){
                ...
                var htmlstr = "<div class='blockdate'><span>"+Mwosctranslator.translate('Time:')+" </span><span id='clock'>"+hourtime+"</span></div>";
                ...
                var htmlstr = "<div class='blockdate'><span>"+Mwosctranslator.translate('Time:')+" </span><span id='clock'>"+curTime+"</span></div>";
                ...
                window.OneStep.$.ajax({
                    ...
                    success: function(msg){
                        if(msg){
                            ...
                            window.OneStep.$('#changedate').html("<span style='width:104px;margin-right:35px;'>"+Mwosctranslator.translate("time off this day")+"</span>");
                        }
                    }
                });
            }
