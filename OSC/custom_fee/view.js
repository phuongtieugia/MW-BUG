...
window.OneStep.$(document).ready(function($){
    OneStep.Views.Payment           = Backbone.View.extend({
        ...
        hdlChangeMethod: function(ev){
            ...
            if(val != this.payment_method_changed){
                ...
                var mw_fee = window.OneStep.$('#id_mw_fee').is(':checked');
                var params = {updategiftwrap: temp,updatemwfee: mw_fee,updatepaymenttype: (window.OneStep.$.inArray(val, regetItemPayment) == -1 ? false : true), updatepaymentmethod: (window.OneStep.$.inArray(val, regetItemPayment) == -1 ? false : true)};
    ...
    OneStep.Views.Cart              = Backbone.View.extend({
        ...
        hdlMwFee : function(){
            var temp = window.OneStep.$('#id_mw_fee').is(':checked');
            var params = {updatemwfee: temp};
            view_onestep_init.update(params);
        }
    });
...
//          this.initFbApp();
    ...
    OneStep.Views.LoadDataForm      = Backbone.View.extend({
        ...
        checkIsMwfee : function(){
            var temp = window.OneStep.$('#check_session_mwfee').val();
            if(temp=='0'){
                window.OneStep.$('#id_mw_fee').prop("checked",false);
            }else{
                window.OneStep.$('#id_mw_fee').prop("checked",true);
            }
        },
        billing: function(){
            if(!isLogged){
