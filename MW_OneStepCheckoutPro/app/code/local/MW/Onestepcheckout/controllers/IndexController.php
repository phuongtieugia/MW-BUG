...
class MW_Onestepcheckout_IndexController extends Mage_Checkout_OnepageController
{
    ...
    public function updateordermethodAction()
    {
        ...
        if($this->getRequest()->isPost()) {
            ...
            if(empty($data_save_shipping_method)) {
                echo json_encode(array(
                    'error' => 1,
                    'msg'   => Mage::helper('onestepcheckout')->__('Shipping method must be selected')
                ));
                exit;
            }
            else{
        ...
        if(empty($data_savepayment)) {
            echo json_encode(array(
                'error' => 1,
                'msg'   => Mage::helper('onestepcheckout')->__('Payment method must be selected')
            ));
            exit;
        } else {
            try {
