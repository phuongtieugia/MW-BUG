...
class MW_RewardPoints_Model_Quote_Address_Total_Rewardpoints_After extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
        ...
        if ((int)Mage::helper('rewardpoints')->getRedeemPointsOnTax($store_id) == MW_RewardPoints_Model_Redeemtax::AFTER) {
            ...
            foreach ($items as $item) {
                ...
                if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                    ...
                    $reward_point_array = $this->processRule($item, $programs, $qty, $price, $baseSubtotalWithDiscount, $itemsCount);
            ...
            /*Recalculate tax*/
            $discountPercent = ($address->getGrandTotal() - $address->getMwRewardpointDiscountShow())/$address->getGrandTotal();
            $newAppliedTaxes = array();
            $newTaxAmount = 0;
            $newBaseTaxAmount = 0;
            foreach($address->getAppliedTaxes() as $appliedTax){
                $appliedTax['amount']*=$discountPercent;
                $appliedTax['base_amount']*=$discountPercent;
                $newTaxAmount += $appliedTax['amount'];
                $newBaseTaxAmount += $appliedTax['base_amount'];
                $newAppliedTaxes[] = $appliedTax;
            }
            $address->setAppliedTaxes($newAppliedTaxes);
            ...
            if ($address->getSubtotal() > 0) {
                ...
                if ($quote->getMwRewardpoint() <= 0 || $totalDiscountAmount <= 0 || $baseTotalDiscountAmount <= 0) {
                    ...
                    $address->setMwRewardpointDiscount($quote->getMwRewardpointDiscount());
                }
    ...
    public function processRule(Mage_Sales_Model_Quote_Item_Abstract $item, $programs, $qty, $price, $baseSubtotalWithDiscount, $itemsCount = 0)
    {
        $result = array();
...
//                        if (!(isset ($this->array_whole_cart_x_y [$program]) && $this->array_whole_cart_x_y [$program] == 1)) {
                        ...
                        $result_reward_point /= $itemsCount;
                        $_result_detail [$program] /= $itemsCount;
//                        }
                    }
