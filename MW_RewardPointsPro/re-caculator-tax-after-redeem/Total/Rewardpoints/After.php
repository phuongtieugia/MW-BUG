            ...
            else if((int)Mage::helper('rewardpoints')->getRedeemPointsOnTax($store_id) == MW_RewardPoints_Model_Redeemtax::BEFORE)
            {
                if($address->getSubtotal() > 0)
                {
			        ...
			        if ($baseMwRewardpointDiscount == 0) {
			            ...
			            $mwRewardpointDiscount = Mage::helper('core')->currency($baseMwRewardpointDiscount, false, false);
			        }
			        ...
			        if ($baseRedeemTaxAmount == 0) {
			            ...
			            $redeemTaxAmount = Mage::helper('core')->currency($baseRedeemTaxAmount, false, false);
			        }

			        // Set new tax amount after redeem points
			        $address->setTaxAmount($address->getTaxAmount() - $redeemTaxAmount)
			            ->setBaseTaxAmount($address->getBaseTaxAmount() - $baseRedeemTaxAmount)
			            ->setGrandTotal($address->getGrandTotal() - $redeemTaxAmount)
			            ->setBaseGrandTotal($address->getBaseGrandTotal() - $baseRedeemTaxAmount);
