<?php
    class MW_RewardPoints_Helper_Data extends Mage_Core_Helper_Abstract
    {
        ...
        */
            ...
            foreach ($quote->getAllVisibleItems() as $item)
            {
                ...
                if(isset($infoArr['super_attribute'])){
                    foreach ($infoArr['super_attribute'] as $attr_id => $val)
                    {
                        ...
                        $reward_point_cfg += intval($collection->getEarnPoint());
                    }
                }

                $mw_reward_point += $reward_point_cfg;
                if($mw_reward_point > 0)
                {
		...
		public function checkModuleEnabled($store_id)
        {
            return Mage::getStoreConfig('rewardpoints/config/enabled',$store_id);
        }
    }
