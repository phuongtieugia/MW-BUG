...
class MW_RewardPoints_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function exchangePointsToMoneys($rewardpoints, $store_id)
	{
		...
		$grandTotal=number_format(Mage::getSingleton('checkout/session')->getQuote()->getSubtotal(), 0);
		$money = $this->getRwpratesConfig($rewardpoints,$grandTotal);
	...
	*/
	...
	public function getRwpratesConfig($redeem_point,$grandTotal){
		$config_rwprates = Mage::getStoreConfig('rewardpoints/mw_rewardpoints_rate/rwprates');
		$i = 0;
		if ($config_rwprates) {
			$config_rwprates = unserialize($config_rwprates);
			if (is_array($config_rwprates)) {
				foreach($config_rwprates as $config_rwprate) {
					$rwpratesfrom = $config_rwprate['rwpratesfrom'];
					$rwpratesto = $config_rwprate['rwpratesto'];
					if($redeem_point >= $rwpratesfrom && $redeem_point <= $rwpratesto){
						
						if($grandTotal < $config_rwprate['mwrwprate']){
							return $i;
						}
						return $config_rwprate['mwrwprate'];
						// so sanh vs grandTotal, neu lon hon thi return i
					}
					$i = $config_rwprate['mwrwprate'];
				}
			}
		}
		return 0;
