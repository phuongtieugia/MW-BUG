<?php
    class MW_RewardPoints_Helper_Data extends Mage_Core_Helper_Abstract
    {
        ...
        public function getDisplayEarnpointViewProduct($_product)
        {
            ...
            $enable      = $this->moduleEnabled();
            if($mw_reward_point >= 0 && $enable)
            {	$porciones = explode(",", $this->formatPoints($mw_reward_point, $store_id));
				$porcionesf = $porciones[1]/100;
                ...
                //$hidden . $reward_icon . $this->__("You will earn <b>%s</b> for buying this product.", $porcionesf) .
				$hidden . $reward_icon . $this->__("Obtén <b><span id='point-detail'>%s</span> puntos (<span id='money_detail'>%s</span> €)</b> comprando este producto.", $porcionesf*100, $porcionesf) .
                '</span>';
            }
        ...
        public function getRewardpointGoMageTemplate(){
			return Mage::app()->getLayout()->createBlock('rewardpoints/checkout_cart_rewardpoints')->setTemplate('mw_rewardpoints/checkout/onepage/rewardpoints_gomage.phtml')->toHtml();
		}
        ...
        public function getRewardIconHtml($store_id)
        {
            ...
            $reward_icon      = '<span><img style ="vertical-align: middle;" alt="Reward Points Policy" src="' . Mage::getBaseUrl('media') . 'mw_rewardpoint/' . $image_reward_icon . '"></span>';
