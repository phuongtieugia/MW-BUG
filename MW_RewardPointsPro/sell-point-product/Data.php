<?php
    class MW_RewardPoints_Helper_Data extends Mage_Core_Helper_Abstract
    {
                ...
                if($customer_id && $enable)
                {
                    ...
                    if(sizeof($collection_customers) > 0)
                    {
                        foreach ($collection_customers as $collection_customer)
                        {
                            ...
                            {
                                return '<span> (<a href="' . Mage::getUrl("rewardpoints/rewardpoints") . '">' . $this->formatPoints($collection_customer->getMwRewardPoint(), $store_id) . '</a>)</span>';
                            }
            ...
            if($customer_id && $enable)
            {
                ...
                if(sizeof($collection_customers) > 0)
                {
                    foreach ($collection_customers as $collection_customer)
                    {
                        if($collection_customer->getMwRewardPoint() > 0)
                        {
                            return '<span> (' . $this->formatPoints($collection_customer->getMwRewardPoint(), $store_id) . ')</span>';
                        }
                        else
                        {
                            return '';
        ...
        public function exchangePointsToMoneys($rewardpoints, $store_id = null)
        {
            if($rewardpoints < 0)
            {
                return 0;
        ...
        public function exchangeMoneysToPoints($money, $store_id = null)
        {
            if($money < 0)
            {
                return 0;
        ...
        public function setPointToCheckOut($rewardpoints, $quote = null, $customer_id = null, $store_id = null, $baseGrandTotal = null)
        {
            ...
            if($customer_id)
            {
                ...
                {
                    $minPoints = (float)Mage::getStoreConfig('rewardpoints/config/min_checkout');
        ...
        */
        ...
        public function getCheckoutRewardPointsRule($quote, $store_id = null)
        {
            ...
            foreach ($quote->getAllItems() as $item)
            {
                ...
                if($info = $item->getProduct()->getCustomOption('info_buyRequest')) {
                    ...
                    if(count($infoArr) > 0) {
                        $model = Mage::getModel('rewardpoints/productsellpoint');
						if(is_array($infoArr['super_attribute']) || is_object($infoArr['super_attribute'])){
							foreach ($infoArr['super_attribute'] as $attr_id => $val) {
								$collection = $model->getCollection()
									...
									->getFirstItem();
								$mw_reward_point = $mw_reward_point + intval($collection->getEarnPoint());
							}
						}
		...
		// custom by phuongtieugia
		public function getTotalRewardpointSell(){
			// check customer 
			...
			// get customer point
			$collection_customers = Mage::getModel('rewardpoints/customer')->getCollection()->addFieldToFilter('customer_id', $customer_id);
			if(sizeof($collection_customers) > 0)
			{
				foreach ($collection_customers as $collection_customer)
				{
					if($collection_customer->getMwRewardPoint() > 0)
					{
						$collection_customers = $collection_customer->getMwRewardPoint();
					}
					else
					{
						$collection_customers = 0;
					}
					break;
				}
			}
			Mage::getSingleton('customer/session')->setCustomerSellPoints($collection_customers);
			/* $productsellP = $product->getData('mw_reward_point_sell_product'); */
			$mw_inp_sell_point = $product->getData('mw_reward_point_sell_product');
			
			if(Mage::app()->getRequest()->getParam('qty') >= 1 && !count($items)){
				if($collection_customers >= $mw_inp_sell_point)
					return $mw_inp_sell_point;
				else 
					return 0;
			}
			...
			$finalP = $mw_inp_sell_point + $product_sell_point;
			Mage::getSingleton('customer/session')->setProductSellPoints($product_sell_point);
			if($collection_customers >= $finalP)
				return $finalP;
			return 0;
			
		}
		public function checkSellPointEnable(){
			$customer_id = Mage::getSingleton('customer/session')->getCustomer()->getId();
            if(!$customer_id)
			{
				return false;
			...
			// end get product sell points
			
			if(isset($product_sell_point) && isset($collection_customers)){
				if($collection_customers < $product_sell_point){
					return true;
				}
			}
			return false;
