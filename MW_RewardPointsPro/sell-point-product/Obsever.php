<?php
    ...
    class MW_RewardPoints_Model_Obsever
    {
		...
		public function checkoutCartProductAddBefore($observer){
			Mage::log("tsssts",null,"ptg.log");
			$store_id = Mage::app()->getStore()->getId();
			$request = Mage::app()->getRequest()->getPost();
			$productId = Mage::app()->getRequest()->getParam('product');
			$store_product = Mage::getModel('catalog/product')->load($productId); 
			$productsellP = $store_product->getData('mw_reward_point_sell_product');
			// check mw_inp_sell_point
			if(isset($productsellP) && $productsellP > 0){
				$sell_point = Mage::helper("rewardpoints")->getTotalRewardpointSell();
				if($sell_point == 0){
					
					Mage::app()->getResponse()->setRedirect($store_product->getProductUrl());
					Mage::getSingleton('core/session')->addNotice('Sorry, this product is currently not available. Could you please login to buy this product');
